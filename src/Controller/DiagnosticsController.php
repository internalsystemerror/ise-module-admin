...
class DiagnosticsController extends AbstractActionController
{
     ...
     * Constructor
     *
     * @param AdapterInterface $console        Console adapter
     * @param array            $config         Application configuration
     * @param ModuleManager    $moduleManager  ZF module manager
     * @param ServiceLocator   $serviceLocator Application service locator
     */
    public function __construct($console, array $config, ModuleManager $moduleManager, ServiceLocatorInterface $serviceLocator)
    {
    ...
    public function runAction()
    {
        ...
        $this->config = isset($this->config['diagnostics']) ? $this->config['diagnostics'] : array();
        ...
        if ($checkGroupName) {
            $this->config = array_intersect_ukey($this->config, array($checkGroupName => 1), 'strcasecmp');

            if (empty($this->config)) {
                $m = new ConsoleModel();
                $m->setResult($this->console->colorize(sprintf(
        ...
        foreach ($this->config as $checkGroupName => $checks) {
